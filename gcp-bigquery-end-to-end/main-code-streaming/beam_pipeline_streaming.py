...
delivered_table_spec = 'strong-bus-402615:dataset_food_orders_streaming.delivered_orders'
other_table_spec = 'strong-bus-402615:dataset_food_orders_streaming.other_status_orders'
...
options.view_as(StandardOptions).streaming = True # Added here for streaming
...
cleaned_data = (
    ...
    | beam.io.ReadFromPubSub(topic=inputs_pattern)# ReadFromText(inputs_pattern, skip_header_lines=1)# Changed here for streaming # inputs_pattern is the object containing input file location. 
...
# For all 3 blocks below: no longer need for streaming. Since streaming is unbounded data. 
'''
(cleaned_data
...
dataset_id = "{}.dataset_food_orders_streaming".format(client.project)
...
# This view_creation block below is moved from the bottom to here. See bottom for details
def create_view():
    ...
    dataset_ref = client.dataset('dataset_food_orders_streaming')
    ...
    view_to_create.view_query = 'SELECT * FROM `strong-bus-402615:dataset_food_orders_streaming.delivered_orders`'
...
# call this method from a new thread (to avoid on-going streaming data never finishes and thus view never gets created)
from threading import Timer
t=Timer(10.0, create_view) # After 10 sec, create_view will be ran in a new thread, independant of the current execution. Set as 10 secs since there should at least be some data created in the table, before you can create view on top of it.
t.start()
...
res = p.run()
# Added below for streaming
res.wait_until_finish() # This allows the pipeline to run continuously on the cloud, but the local code waits for the cloud job to finish and return the final status
...
# Problem with streaming: since it's streaming, data continues to come in, and it won't end until it finishes, so the view_creation job below won't be ran. 
# To resolve this, move this block to above, using another thread.
...
# Final note: when running this from cloud shell, run: 
# python3 beam_pipeline_streaming.py --input projects/strong-bus-402615/topic s/mimic_data_streaming_v2 --temp_location gs://myfirstbucket_jw
